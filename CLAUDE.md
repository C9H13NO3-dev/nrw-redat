...
26-card `SECTIONS` registry (`Section(key, title, icon, tier, timeout_s, source, fetch)`); `core/envelope.py`'s
...
- New static data lives in `redat/data/` and is committed (like `zensus_2022_grid.json.gz`); `schulen_nrw.json.gz` and `unfallatlas_2020_2025.json.gz` are the two Tier-1 additions — see README "Static grids in the repo" for the build script and refresh cadence per file.
