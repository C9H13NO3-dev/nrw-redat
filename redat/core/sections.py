...
def _fetch_unfaelle(ctx: Ctx) -> dict:
    ...
    if d is None:
        raise Empty("Keine Unfallatlas-Daten für diesen Ort (außerhalb Nordrhein-Westfalens oder Datei fehlt)")
    return d
...
def _fetch_bergbau(ctx: Ctx) -> dict:
    ...
    if b["bodenbewegung"] is None:
        b["bodenbewegung_hinweis"] = "EGMS-Bodenbewegungsdaten nicht installiert oder keine Messzelle an diesem Punkt (siehe scripts/build_egms.py)"
    return b
...
SECTIONS: dict[str, Section] = {s.key: s for s in [
    ...
    Section("bergbau", "Bergbau & Untergrund", "⛏️", 20, "Geologischer Dienst NRW, „NRW von unten“ (Bürgerversion, 500 m-Planquadrat) · Bergbauberechtigungen NRW (BezReg Arnsberg) · Copernicus EGMS Bodenbewegung", _fetch_bergbau, cache_version=4),
    ...
    Section("unfaelle", "Verkehrsunfälle (Unfallatlas)", "🚧", 10,
            "Statistische Ämter des Bundes und der Länder, Unfallatlas 2020–2025 (dl-de/by-2-0) — nur Unfälle mit Personenschaden — NRW-weit", _fetch_unfaelle,
            cache_version=2),
